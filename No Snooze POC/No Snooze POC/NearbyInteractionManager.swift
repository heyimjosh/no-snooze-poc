...
@globalActor final actor NearbyInteractionManager: NSObject, GlobalActor {
  ...
  // Does this imply you can connect to only one NearbyInteraction instance?
...
//    initializeNISession()
  ...
  func startTask() -> AsyncThrowingStream<Measurement<UnitLength>, Error> {
...
//    session = NISession()
//    session?.delegate = niSessionDelegate(restartNISession: restartNISession, deinitializeNISession: deinitializeNISession)
//    session?.delegateQueue = DispatchQueue.main
//  }
...
//  }
//  
//  func session(_ session: NISession, didRemove nearbyObjects: [NINearbyObject], reason: NINearbyObject.RemovalReason) async {
//    switch reason {
//    case .peerEnded:
//      os_log("the remote peer ended the connection")
//      deinitializeNISession()
